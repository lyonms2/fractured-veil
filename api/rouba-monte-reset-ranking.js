// ═══════════════════════════════════════════════════════════════════
// ROUBA MONTE — Reset semanal do ranking + distribuição da pool
// Vercel Cron Job: toda segunda-feira às 00:00 UTC
// vercel.json: { "crons": [{ "path": "/api/rouba-monte-reset-ranking", "schedule": "0 0 * * 1" }] }
// ═══════════════════════════════════════════════════════════════════
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    for(const fila of filas) {
      ...
      // Busca top 10 do ranking do Rouba Monte
      const rankSnap = await rtdb.ref(`roubaMonte/ranking/${fila.nome}`)
        .orderByChild('pontos')
        .limitToLast(10)
        .once('value');
        ...
        try {
          ...
          if(fila.usaCristais) {
            await db.collection('config').doc('pool').collection('logs').add({
              tipo:   'saida',
              motivo: `Rouba Monte ${fila.nome} — ${i+1}º lugar semana ${semana}`,
              origem:  wallet,
              total:   premio,
              pool:    -premio,
          ...
          // Notificação no RTDB
          await rtdb.ref(`roubaMonte/notificacoes/${wallet}/premiacoes`).push({
            semana,
            fila:    fila.nome,
            posicao: i + 1,
            premio,
    ...
    console.error('rouba-monte-reset-ranking erro:', e);
...
async function _resetarRanking(log) {
  for(const fila of ['Comum', 'Raro', 'Lendário']) {
    await rtdb.ref(`roubaMonte/ranking/${fila}`).remove();
    log.push(`Ranking Rouba Monte ${fila} resetado.`);
  }
}
