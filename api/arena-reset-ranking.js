// ═══════════════════════════════════════════════════════════════════
// ARENA — Reset semanal do ranking + distribuição da pool
// Vercel Cron Job: toda segunda-feira às 00:00 UTC
// vercel.json: { "crons": [{ "path": "/api/arena-reset-ranking", "schedule": "0 0 * * 1" }] }
// ═══════════════════════════════════════════════════════════════════
...
const SPLIT_RARO     = 0.40; // 40% do bolo vai para Raro
// Comum não recebe cristais da pool — recebe moedas internas geradas pelo sistema

// Moedas para a fila Comum (não saem da pool de cristais)
const MOEDAS_COMUM_TOTAL = 10000;

// Distribuição percentual por posição dentro de cada fila
// Soma deve ser exatamente 1.00
...
// Mínimo de pontos para receber premiação (evita prêmio por 1 partida)
const MIN_PONTOS = 5;
...
module.exports = async (req, res) => {
  // Segurança — só o Vercel Cron pode chamar (via CRON_SECRET)
  ...
  try {
    ...
    const boloRaro     = boloSemanal - boloLendario; // pega o resto para não perder 1 cristal por arredondamento

    log.push(`Bolo semanal (${PCT_POOL_SEMANAL*100}% da pool): ${boloSemanal} 💎`);
    log.push(`  → Lendário (${SPLIT_LENDARIO*100}%): ${boloLendario} 💎`);
    log.push(`  → Raro (${SPLIT_RARO*100}%): ${boloRaro} 💎`);
    ...
    for(const fila of filas) {
      ...
      const rankSnap = await rtdb.ref(`arena/ranking/${fila.nome}`)
        .orderByChild('pontos')
        .limitToLast(10)
        .once('value');
      ...
      for(let i = 0; i < lista.length; i++) {
        ...
        try {
          // Credita no Firestore do jogador
          ...
          // Log na pool (só para cristais — moedas não saem da pool)
          if(fila.usaCristais) {
            await db.collection('config').doc('pool').collection('logs').add({
              tipo:   'saida',
              motivo: `Arena ${fila.nome} — ${i+1}º lugar semana ${semana}`,
              origem:  wallet,
              total:   premio,
              pool:    -premio,
          ...
          // Notificação no RTDB para o jogador ver ao entrar no jogo
          await rtdb.ref(`arena/notificacoes/${wallet}/premiacoes`).push({
            semana,
            fila:    fila.nome,
            posicao: i + 1,
            premio,
    ...
    console.error('arena-reset-ranking erro:', e);
...
async function _resetarRanking(log) {
  for(const fila of ['Comum', 'Raro', 'Lendário']) {
    await rtdb.ref(`arena/ranking/${fila}`).remove();
    log.push(`Ranking ${fila} resetado.`);
  }
}
